...
//  
//
//  Created by Johan Lipecki on 2016-05-04.
//
//
...
int main(void)
{
    IPaddress ip;
    TCPsocket sd;
    char newport[MAXLEN], SYN0[MAXLEN] = "hearts", SYN1[MAXLEN] = "port", error_log_string[40], pid[7];
    int result, len, len2, fd;
    ...
    if (SDLNet_ResolveHost(&ip, IP_ADDRESS, PORT) < 0)
    {
