...
#define EXPECTED_REPLY "diamonds"
#define PROMPT  "port"
...
int main(void)
{
    IPaddress ip;
    TCPsocket sd;
    char server[25] = "", newport[MAXLEN], SYN1[MAXLEN] = "Grupp7;1234", SYN0[MAXLEN] = "hearts", error_log_string[40], pid[7];
    ...
    if(strcmp(SYN0, EXPECTED_REPLY) == 0)
    {
        ...
        */
        ...
        if(result=SDLNet_TCP_Recv(sd,SYN1,MAXLEN)) fprintf(fd,"received port: %s\nstring length: %d",SYN1,result);
        while(strcat(newport, strsep(&SYN1,";")))if(!printf(SYN1))break;
